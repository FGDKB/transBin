...
_KERNEL = _transBintools.read_npz(_os.path.join(_os.path.dirname(_os.path.abspath(__file__)),
                              "kernel.npz"))
...
def read_contigs(filehandle, minlength=100):
    ...
    raw = _transBintools.PushArray(_np.float32)
    projected = _transBintools.PushArray(_np.float32)
    lengths = _transBintools.PushArray(_np.int32)
    contignames = list()

    entries = _transBintools.byte_iterfasta(filehandle)
