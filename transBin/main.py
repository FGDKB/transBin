...
def trainvae(outdir, rpkms, tnfs, nlatent, alpha, beta, dropout, cuda,
             batchsize, nepochs, lrate, batchsteps, logfile):
    begintime = time.time()
    log('\nCreating and training VAE', logfile)

    nsamples = rpkms.shape[1]
    transformer = transBin.test.Transformer(nsamples, nlatent=nlatent,
                          alpha=alpha, beta=beta, dropout=dropout, cuda=cuda)

    log('Created transformer', logfile, 1)
    dataloader, mask = transBin.test.make_dataloader(rpkms, tnfs, batchsize,
    ...
    transformer.trainmodel(dataloader, nepochs=nepochs, lrate=lrate, batchsteps=batchsteps,
